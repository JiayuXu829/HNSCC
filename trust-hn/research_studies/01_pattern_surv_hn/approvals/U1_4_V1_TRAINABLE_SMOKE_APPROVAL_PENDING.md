# U1.4/V1 Trainable Smoke Approval Request
...
- [ ] Confirm PyTorch 2.12.1+cpu was installed only in the project `.venv` and frozen in the U1.4 directory.
- [ ] Confirm `pyproject.toml` and all registered Phase 6 decision files remain unchanged.
- [ ] Confirm U1.4 inherits the frozen U1.3 dimensions and contains 3,225 trainable parameters.
- [ ] Confirm gradients are finite/nonzero and parameters update under Adam optimization.
- [ ] Confirm synthetic Cox loss decreased from 3.60724 to 0.605562 (83.21% relative reduction).
- [ ] Confirm modality-order permutation invariance holds before and after optimization.
- [ ] Confirm no active modality token still yields exactly `delta_eta = 0` after optimization.
- [ ] Confirm the fused score still equals the clinical score exactly under empty-set fallback.
- [ ] Confirm two formal smoke runs produced the same aggregate audit SHA256.
- [ ] Confirm no HANCOCK patient training, formal development CV, official-test/external evaluation, router, or calibrator occurred.
- [ ] Confirm these results establish trainability only, not prognostic performance or fusion benefit.
